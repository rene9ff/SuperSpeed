...
checkroot() {
  [[ $EUID -ne 0 ]] && echo -e "${RED}请切换root用户！${PLAIN}" && exit 1
}
...
selecttest() {
  echo -e "  类型:    ${GREEN}1.${PLAIN} 三网    ${GREEN}2.${PLAIN} 取消"
  echo -ne "               ${GREEN}3.${PLAIN} 电信    ${GREEN}4.${PLAIN} 联通    ${GREEN}5.${PLAIN} 移动"
  while :; do
    echo
    read -p "  请选择类型: " selection
...
runtest() {
  ...
  if [[ ${selection} == 1 ]]; then
    echo "——————————————————————————————————————————————————————————"
    echo "ID      服务器信息       上传/Mbps   下载/Mbps   延迟/ms"
    start=$(date +%s)
    ...
    # 添加自 Oldking 脚本
    # speed_test '4595' '河南郑州O' '电信'
    # speed_test '12637' '湖北襄阳O' '电信'
    speed_test '10305' '广西南宁O' '电信'
    # speed_test '10305' '江西南昌O' '电信'
    speed_test '6132' '湖南长沙O' '电信'
    speed_test '5081' '广东深圳O' '电信'
    speed_test '6592' '重庆O' '电信'
    speed_test '4624' '四川成都O' '电信'
